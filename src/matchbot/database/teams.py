...
class TeamsTable:
    ...
    async def add(self, *teams: Team):
        await self.db.executemany("INSERT INTO teams(id, name, tag)"
                                  " VALUES ($1, $2, $3)",
    ...
    async def get(self, column: str, *values) -> Union[Team, Iterable[Team]]:
        ...
        teams = [Team(name=record.get('name'),
                      tag=record.get('tag'),
                      id=record.get('id'))
                 for record in await self.db.fetch("SELECT id, name, tag"
                                                   " FROM teams"
        ...
        for team in teams:
            team.players = [User(steam_id=record.get('steam_id'),
                            ...
                            for record in
                            await self.db.fetch("SELECT steam_id, discord_id, display_name"
                                                " FROM users"
    ...
    async def get_by_name(self, *names):
        return await self.get('name', *names)

    async def get_by_tag(self, *tags):
