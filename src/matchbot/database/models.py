    ...
    def config(self):
        ...
        return json.dumps({'matchid': str(self.id),
                           ...
                           'team1': {'name': self.team1.name,
                                     'tag': self.team1.tag,
                                     'players': {str(user.steam_id): user.display_name for user in self.team1.users}},
                           'team2': {'name': self.team2.name,
                                     'tag': self.team2.tag,
                                     'players': {str(user.steam_id): user.display_name for user in self.team2.users}},
...
if __name__ == '__main__':
    ...
    load_dotenv('../../.env')
    engine = new_engine('localhost', int(getenv('POSTGRES_PORT')), getenv('POSTGRES_USER'), getenv('POSTGRES_PASSWORD'),
                        getenv('POSTGRES_DB'))
